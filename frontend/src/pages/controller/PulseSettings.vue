...
<template>
  <div class="w-full">
    ...
    <TransitionGroup v-if="parentState.pulseList" name="list-pop" tag="div" appear
      class="grid justify-center grid-cols-1 md:grid-cols-2 gap-4 pb-2">
      <PulseCard v-for="(pulse, index) in fullPulseList" :key="pulse.id" :pulse-info="pulse"
        :is-current-pulse="parentState.selectPulseIds.includes(pulse.id)"
        :is-fire-pulse="pulse.id === parentState.firePulseId" :style="{ '--stagger-i': index }"
        @set-current-pulse="togglePulse" @set-fire-pulse="setFirePulse" @delete-pulse="handleDeletePulse"
        @rename-pulse="handleRenamePulse" />
    </TransitionGroup>
