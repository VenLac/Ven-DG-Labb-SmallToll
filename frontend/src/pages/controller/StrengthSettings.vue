...
<template>
  <div class="w-full" style="--enter-base-delay: 160ms;">
    ...
    <div class="w-full flex flex-col md:flex-row items-top lg:items-center gap-2 lg:gap-8 mb-8 lg:mb-4">
      <label class="font-semibold w-35">A通道最大电量</label>
      <InputNumber class="input-small" v-model="parentState.channelAStrengthLimit" :min="1" :max="200"
        placeholder="不限" />
      <div class="flex-grow flex-shrink"></div>
    </div>
    <div class="w-full flex flex-col md:flex-row items-top lg:items-center gap-2 lg:gap-8 mb-8 lg:mb-4">
      <label class="font-semibold w-35">B通道最大电量</label>
      <InputNumber class="input-small" :disabled="!parentState.bChannelEnabled"
        v-model="parentState.channelBStrengthLimit" :min="1" :max="200" placeholder="不限" />
      <div class="flex-grow flex-shrink"></div>
    </div>
    <div class="flex gap-8 mb-4 w-full">
      <div class="w-35"></div>
      <div class="opacity-60 text-right">
        通道最大电量为软件层安全上限，实际输出不会超过该值；留空表示不限（跟随设备硬件上限）
      </div>
    </div>
