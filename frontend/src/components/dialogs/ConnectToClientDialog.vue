...
// 从「连接过的设备」直接重连（走 update:clientId，与「通过客户端ID连接」同一路径）
const onConnectSavedClient = (clientId: string) => {
  emit('update:clientId', clientId);
};

// 从「连接过的设备」直接弹出对应版本的蓝牙配对窗（浏览器限制仍需手动选设备）
const onConnectSavedBluetooth = (version: CoyoteDeviceVersion) => {
  ...
  <Dialog v-model:visible="visible" modal header="连接设备" class="mx-4 w-full md:w-[40rem]">
    <FadeAndSlideTransitionGroup>
      <div v-if="props.clientWsUrlList">
        <div v-if="clientsStore.clientList.length > 0" class="mb-4">
          <div class="font-semibold mb-2">连接过的设备</div>
          <ConnectToSavedClientsList @connect-to-client="onConnectSavedClient"
            @connect-bluetooth="onConnectSavedBluetooth" />
        </div>
