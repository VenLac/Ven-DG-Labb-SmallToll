...
const startBluetoothConnect = async (deviceVersion: CoyoteDeviceVersion) => {
  ...
  try {
    ...
    const connectorType = deviceVersion === CoyoteDeviceVersion.V2 ? ConnectorType.COYOTE_BLE_V2 : ConnectorType.COYOTE_BLE_V3;
    parentState.connectorType = connectorType;

    // 记住蓝牙设备，便于下次从「连接过的设备」一键弹出对应版本的配对窗
    const controller = state.controller;
    const deviceName = (controller instanceof CoyoteBluetoothController ? controller.device?.name : undefined)
      || (deviceVersion === CoyoteDeviceVersion.V2 ? '蓝牙设备 2.0' : '蓝牙设备 3.0');
