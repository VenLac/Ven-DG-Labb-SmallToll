...
```sh
POST /api/v2/game/{clientId}/channel_limit
```
...
设置A、B通道各自的最大输出电量（软件层安全上限）。设置后，普通输出、一键开火、插件API调用的实际强度都不会超过该值。```null```表示不限（跟随设备硬件上限）。

使用JSON POST格式发送请求的Post Body：

```json5
{
    "channelAStrengthLimit": 50, // A通道最大电量，null表示不限
    "channelBStrengthLimit": 30  // B通道最大电量，null表示不限
}
```

两个字段均为可选，未传入的字段保持原值不变，至少需要传入一个字段。
...
通道最大电量也会包含在```GET /api/v2/game/{clientId}```返回的```gameConfig```字段中（```channelAStrengthLimit```、```channelBStrengthLimit```）。MCP客户端可调用```set_channel_strength_limit```工具进行相同的设置。
