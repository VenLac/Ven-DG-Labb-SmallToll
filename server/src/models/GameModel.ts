...
export class GameModel implements MainGameConfig {
    ...
    @Column({ type: 'int', name: 'channel_a_strength_limit', nullable: true, comment: 'A通道最大电量，null表示不限（跟随设备硬件上限）' })
    channelAStrengthLimit!: number | null;

    @Column({ type: 'int', name: 'channel_b_strength_limit', nullable: true, comment: 'B通道最大电量，null表示不限（跟随设备硬件上限）' })
    ...
    public static getDefaultConfig(): GameModel {
        ...
        model.channelAStrengthLimit = null;
        model.channelBStrengthLimit = null;
