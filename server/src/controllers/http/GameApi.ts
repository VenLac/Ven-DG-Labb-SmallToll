...
import { GameStrengthConfig, MainGameConfig, MCPunishmentConfig } from '#app/types/game.js';
...
import { GameMCDamageAction } from '../game/actions/GameMCDamageAction.js';
...
import {
    ...
    SetChannelLimitRequest,
    SetChannelLimitRequestSchema,
    SetConfigResponse,
    ...
    MCDamageRequest,
    MCDamageRequestSchema,
    MCPunishmentConfigRequest,
    MCPunishmentConfigRequestSchema,
    MCPunishmentConfigResponse,
    MCPunishmentConfigResponseSchema,
...
import { MCPunishmentModel } from '#app/models/MCPunishmentModel.js';
...
export class GameApiController {
    ...
    /**
     * 设置通道最大电量（A/B通道各自的最大输出强度，null表示不限）
     * @param ctx
     */
    @routeConfig({
        method: 'post',
        path: '/api/v2/game/{id}/channel_limit',
        summary: '设置通道最大电量',
        operationId: 'Set Game Channel Strength Limit V2',
        tags: ['Game V2'],
        request: {
            params: z.object({
                id: ClientIdSchema,
    ...
    @body(SetChannelLimitRequestSchema)
    @responses(SetConfigResponseSchema)
    public async setChannelLimit(ctx: RouterContext): Promise<void> {
        if (!ctx.params.id) {
            apiResponse(ctx, {
                status: 0,
                code: 'ERR::INVALID_CLIENT_ID',
        ...
        let postBody: SetChannelLimitRequest;
        try {
            postBody = SetChannelLimitRequestSchema.parse(ctx.request.body);
        } catch (err: any) {
            apiResponse(ctx, {
                status: 0,
                code: 'ERR::INVALID_REQUEST',
        ...
        // 只更新显式传入的字段，未传入的通道保持不变
        const updateData: { channelAStrengthLimit?: number | null; channelBStrengthLimit?: number | null } = {};
        if (postBody.channelAStrengthLimit !== undefined) {
            updateData.channelAStrengthLimit = postBody.channelAStrengthLimit;
        }
        if (postBody.channelBStrengthLimit !== undefined) {
            updateData.channelBStrengthLimit = postBody.channelBStrengthLimit;
        }
        ...
        if (Object.keys(updateData).length === 0) {
            apiResponse(ctx, {
                status: 0,
                code: 'ERR::INVALID_REQUEST',
                message: '无效的请求，需要至少一个参数：channelAStrengthLimit 或 channelBStrengthLimit',
            });
            return;
        }

        let clientIdList: string[] = [];
        if (ctx.params.id === 'all') { // 广播模式，设置所有游戏的通道最大电量
            if (!MainConfig.value.allowBroadcastToClients) {
                apiResponse(ctx, {
                    status: 0,
                    code: 'ERR::BROADCAST_NOT_ALLOWED',
        ...
        let successClientIds = new Set<string>();
        for (const clientId of clientIdList) {
            await GameModel.update(ctx.database, clientId, updateData);
            successClientIds.add(clientId);
        }
        ...
        apiResponse(ctx, {
            status: 1,
            code: 'OK',
            message: `成功设置了 ${successClientIds.size} 个游戏的通道最大电量`,
            successClientIds: Array.from(successClientIds),
        } as SetConfigResponse);
    }
    ...
    /**
     * 解析路由参数中的客户端ID，返回目标游戏列表。
     * `all` 走广播（需 allowBroadcastToClients），返回 null 表示已写入错误响应。
     */
    private resolveGameList(ctx: RouterContext): CoyoteGameController[] | null {
        if (ctx.params.id === 'all') {
            if (!MainConfig.value.allowBroadcastToClients) {
                apiResponse(ctx, {
                    status: 0,
            ...
            return Array.from(CoyoteGameManager.instance.getGameList());
        }
    ...
    @routeConfig({
        method: 'post',
        path: '/api/v2/game/{id}/module/mc/damage',
        summary: '上报 MC 受伤惩罚',
        operationId: 'Report MC Damage V2',
        tags: ['Game Module V2'],
        request: {
            params: z.object({
                id: ClientIdSchema,
    ...
    @body(MCDamageRequestSchema)
    @responses(SetConfigResponseSchema)
    public async reportMCDamage(ctx: RouterContext): Promise<void> {
        if (!ctx.params.id) {
            apiResponse(ctx, { status: 0, code: 'ERR::INVALID_CLIENT_ID', message: '无效的客户端ID' });
            return;
        }

        let postBody: MCDamageRequest;
        try {
            postBody = MCDamageRequestSchema.parse(ctx.request.body ?? {});
        } catch (err: any) {
            apiResponse(ctx, { status: 0, code: 'ERR::INVALID_REQUEST', message: `无效的请求，参数错误: ${err.message}` });
            return;
        }

        const gameList = this.resolveGameList(ctx);
        if (!gameList) {
            return;
        }
        ...
        for (const game of gameList) {
            const cfg = game.mcConfig;
            if (postBody.damage < cfg.minDamage) {
                continue; // 低于阈值的伤害忽略
            }

            await game.startAction(new GameMCDamageAction({
                mode: cfg.mode,
                baseStrength: cfg.baseStrength,
                strengthAddend: postBody.damage * cfg.punishRate,
                maxStrength: cfg.maxStrength,
                punishTime: cfg.punishTime * 1000,
            }));
            successClientIds.add(game.clientId);
        }
        ...
        apiResponse(ctx, {
            status: 1,
            code: 'OK',
            message: `成功向 ${successClientIds.size} 个游戏发送了 MC 受伤惩罚`,
            successClientIds: Array.from(successClientIds),
            warnings,
        } as SetConfigResponse);
    }
    ...
    @routeConfig({
        method: 'get',
        path: '/api/v2/game/{id}/module/mc/config',
        summary: '获取 MC 受伤惩罚配置',
        operationId: 'Get MC Punishment Config V2',
        tags: ['Game Module V2'],
        request: {
            params: z.object({
                id: ClientIdSchema,
    ...
    @responses(MCPunishmentConfigResponseSchema)
    public async getMCPunishmentConfig(ctx: RouterContext): Promise<void> {
        if (!ctx.params.id || ctx.params.id === 'all') {
            apiResponse(ctx, { status: 0, code: 'ERR::INVALID_CLIENT_ID', message: '获取配置需要指定具体的客户端ID' });
            return;
        }

        const config = await MCPunishmentModel.getOrCreateByGameId(ctx.database, ctx.params.id);
        apiResponse(ctx, {
            status: 1,
            code: 'OK',
            config,
        } as MCPunishmentConfigResponse);
    }

    @routeConfig({
        method: 'post',
        path: '/api/v2/game/{id}/module/mc/config',
        summary: '设置 MC 受伤惩罚配置',
        operationId: 'Set MC Punishment Config V2',
        tags: ['Game Module V2'],
        request: {
            params: z.object({
                id: ClientIdSchema,
    ...
    @body(MCPunishmentConfigRequestSchema)
    @responses(SetConfigResponseSchema)
    public async setMCPunishmentConfig(ctx: RouterContext): Promise<void> {
        if (!ctx.params.id) {
            apiResponse(ctx, { status: 0, code: 'ERR::INVALID_CLIENT_ID', message: '无效的客户端ID' });
            return;
        }

        let postBody: MCPunishmentConfigRequest;
        try {
            postBody = MCPunishmentConfigRequestSchema.parse(ctx.request.body ?? {});
        } catch (err: any) {
            apiResponse(ctx, { status: 0, code: 'ERR::INVALID_REQUEST', message: `无效的请求，参数错误: ${err.message}` });
            return;
        }

        const updateData: Partial<MCPunishmentConfig> = {};
        ...
        if (Object.keys(updateData).length === 0) {
            apiResponse(ctx, { status: 0, code: 'ERR::INVALID_REQUEST', message: '无效的请求，未提供可更新的字段' });
            return;
        }
        ...
        if (ctx.params.id === 'all') {
            if (!MainConfig.value.allowBroadcastToClients) {
                apiResponse(ctx, { status: 0, code: 'ERR::BROADCAST_NOT_ALLOWED', message: '当前服务器配置不允许向所有客户端广播指令' });
                return;
            }
            for (const game of CoyoteGameManager.instance.getGameList()) {
                clientIdList.push(game.clientId);
            }
        } else {
            clientIdList.push(ctx.params.id);
        ...
        const successClientIds = new Set<string>();
        for (const clientId of clientIdList) {
            const updated = await MCPunishmentModel.update(ctx.database, clientId, updateData);
            if (updated) {
                successClientIds.add(clientId);
            }
        }
        ...
        apiResponse(ctx, {
            status: 1,
            code: 'OK',
            message: `成功设置了 ${successClientIds.size} 个游戏的 MC 受伤惩罚配置`,
            successClientIds: Array.from(successClientIds),
        } as SetConfigResponse);
    }
