...
import {
    ...
    SetChannelStrengthLimitParamsSchema,
...
export class MCPApiController {
    ...
    private static async handleInitialize(ctx: RouterContext, params: any) {
        ...
        return {
            ...
            serverInfo: {
                name: "venwolf-mcp-server",
                title: "VenWolf MCP Server",
                version: "2.0.0"
            },
    ...
    private static async handleToolsList(ctx: RouterContext, specifiedGame: boolean = false) {
        ...
        tools.push(...[
            ...
            {
                name: "set_channel_strength_limit",
                description: "设置A/B通道的最大电量（最大输出强度），作为安全上限。不传或传null表示不限（跟随设备硬件上限）。设置后普通输出、一键开火、插件调用的强度都不会超过该值",
                inputSchema: {
                    type: "object" as const,
                    properties: {
                        channelAStrengthLimit: {
                            type: "number",
                            description: "A通道最大电量(0-200)，null表示不限",
                            minimum: 0,
                            maximum: 200
                        },
                        channelBStrengthLimit: {
                            type: "number",
                            description: "B通道最大电量(0-200)，null表示不限",
                            minimum: 0,
                            maximum: 200
                        }
                    }
        ...
        try {
            ...
            switch (name) {
                ...
                case "fire_action":
                    ...
                    break;
                case "set_channel_strength_limit":
                    result = await this.handleSetChannelStrengthLimit(ctx, session, {
                        channelAStrengthLimit: args.channelAStrengthLimit,
                        channelBStrengthLimit: args.channelBStrengthLimit
                    });
                    break;
                case "get_pulse_list":
    ...
    private static getStrengthStatusMessage(game: CoyoteGameController, operation: string, oldStrength?: number, newStrength?: number): string {
        ...
        const limit = game.effectiveChannelALimit;
    ...
    private static async handleGetGameStatus(ctx: RouterContext, session: MCPConnection, params: any): Promise<GameStatus> {
        ...
        const status: GameStatus = {
            ...
            strengthLimit: game?.effectiveChannelALimit || 0,
     ...
     * 设置通道最大电量
     */
    private static async handleSetChannelStrengthLimit(ctx: RouterContext, session: MCPConnection, params: any) {
        const validation = this.validateGame(session.gameId!);
        if (!validation.valid) {
            throw validation.error;
        }

        const { channelAStrengthLimit, channelBStrengthLimit } = SetChannelStrengthLimitParamsSchema.parse(params);

        // 只更新显式传入的字段，未传入的通道保持不变
        const updateData: { channelAStrengthLimit?: number | null; channelBStrengthLimit?: number | null } = {};
        if (channelAStrengthLimit !== undefined) {
            updateData.channelAStrengthLimit = channelAStrengthLimit;
        }
        if (channelBStrengthLimit !== undefined) {
            updateData.channelBStrengthLimit = channelBStrengthLimit;
        }

        if (Object.keys(updateData).length === 0) {
            throw {
                code: MCP_ERROR_CODES.INVALID_PARAMS,
                message: '需要至少一个参数：channelAStrengthLimit 或 channelBStrengthLimit'
            };
        }

        await GameModel.update(ctx.database, session.gameId!, updateData);

        const fmt = (changed: boolean, v: number | null) => changed ? (v === null ? '不限' : `${v}`) : '未变更';

        return {
            success: true,
            channelAStrengthLimit: updateData.channelAStrengthLimit ?? null,
            channelBStrengthLimit: updateData.channelBStrengthLimit ?? null,
            message: `已设置通道最大电量：A=${fmt('channelAStrengthLimit' in updateData, updateData.channelAStrengthLimit ?? null)}，B=${fmt('channelBStrengthLimit' in updateData, updateData.channelBStrengthLimit ?? null)}。`
        };
    }
