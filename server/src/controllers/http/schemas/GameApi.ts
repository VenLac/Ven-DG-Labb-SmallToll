import { GameStrengthConfigSchema, MainGameConfigSchema, MCPunishmentConfigSchema } from "#app/types/game.js";
...
export const SetChannelLimitRequestSchema = z.object({
    channelAStrengthLimit: AutoCastInt(z.number().int().min(0).max(200)).nullable().optional()
        .describe('A通道最大电量，null或不传表示不限（跟随设备硬件上限）'),
    channelBStrengthLimit: AutoCastInt(z.number().int().min(0).max(200)).nullable().optional()
        .describe('B通道最大电量，null或不传表示不限（跟随设备硬件上限）'),
}).describe('设置通道最大电量请求');
export type SetChannelLimitRequest = z.infer<typeof SetChannelLimitRequestSchema>;
...
export const MCDamageRequestSchema = z.object({
    damage: AutoCastInt(z.number().int().min(0))
        .describe('原始伤害值（HP，1心=2HP）'),
}).describe('MC 受伤惩罚请求');

export type MCDamageRequest = z.infer<typeof MCDamageRequestSchema>;

export const MCPunishmentConfigRequestSchema = MCPunishmentConfigSchema.partial()
    .describe('MC 受伤惩罚配置请求（仅传入需更新的字段）');

export type MCPunishmentConfigRequest = z.infer<typeof MCPunishmentConfigRequestSchema>;

export const MCPunishmentConfigResponseSchema = ApiResponseSchema.extend({
    config: MCPunishmentConfigSchema.nullable().describe('MC 受伤惩罚配置'),
}).describe('MC 受伤惩罚配置响应');

export type MCPunishmentConfigResponse = z.infer<typeof MCPunishmentConfigResponseSchema>;
