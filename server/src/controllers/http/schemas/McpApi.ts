...
export const SetChannelStrengthLimitParamsSchema = z.object({
    channelAStrengthLimit: z.number().int().min(0).max(200).nullable().optional().describe("A通道最大电量，null表示不限"),
    channelBStrengthLimit: z.number().int().min(0).max(200).nullable().optional().describe("B通道最大电量，null表示不限"),
}).describe("设置通道最大电量参数");
