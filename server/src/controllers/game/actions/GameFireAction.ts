...
export class GameFireAction extends AbstractGameAction<GameFireActionConfig> {
    ...
    async execute(ab: AbortController, harvest: () => void, done: () => void): Promise<void> {
        ...
        absoluteStrength = Math.min(this.game.strengthConfig.strength + this.currentFireStrength, this.game.effectiveChannelALimit);
        ...
        let setStrengthInterval = setInterval(() => {
            ...
            if (this.currentFireStrength >= this.fireStrength || absoluteStrength >= this.game.effectiveChannelALimit) {
