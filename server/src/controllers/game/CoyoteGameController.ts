...
import { MainGameConfig, GameStrengthConfig, MCPunishmentConfig } from '#app/types/game.js';
...
export class CoyoteGameController {
    ...
    /** MC 受伤惩罚配置（per-clientId，随数据库更新自动同步） */
    public mcConfig!: MCPunishmentConfig;
    ...
    /** A通道有效最大电量：用户配置与设备硬件上限取较小值，null表示不限（跟随设备上限） */
    public get effectiveChannelALimit(): number {
        const deviceLimit = this.clientStrength.limit;
        const userLimit = this.gameConfig?.channelAStrengthLimit;
        return userLimit != null && userLimit > 0 ? Math.min(userLimit, deviceLimit) : deviceLimit;
    }

    /** B通道有效最大电量：用户配置与设备硬件上限取较小值，null表示不限（跟随设备上限） */
    public get effectiveChannelBLimit(): number {
        // B通道设备上限优先；部分设备/App仅上报A通道上限(B为0)，此时回退到A通道上限，避免B通道被错误钳制为0
        const bDeviceLimit = this.client?.strengthChannelB?.limit;
        const deviceLimit = (typeof bDeviceLimit === 'number' && bDeviceLimit > 0) ? bDeviceLimit : this.clientStrength.limit;
        const userLimit = this.gameConfig?.channelBStrengthLimit;
        return userLimit != null && userLimit > 0 ? Math.min(userLimit, deviceLimit) : deviceLimit;
    }
    ...
    async initialize(): Promise<void> {
        ...
        this.mcConfig = await MCPunishmentModel.getOrCreateByGameId(this.ctx.database, this.clientId);
        ...
        // 监听 MC 受伤惩罚配置更新
        const mcEvents = this.eventStore.wrap(MCPunishmentModel.events);
        mcEvents.on('configUpdated', this.clientId, throttle((newConfig) => {
            this.mcConfig = newConfig;
        }, 100));
    }
    ...
    private handleConfigUpdated(newGameConfig: MainGameConfig): void {
        ...
        let shouldRestartGameKeys = ['pulseId', 'pulseMode', 'pulseChangeInterval', 'strengthChangeInterval', 'enableBChannel', 'bChannelStrengthMultiplier', 'channelAStrengthLimit', 'channelBStrengthLimit'];
    ...
    public async setClientStrength(strength: number): Promise<void> {
        ...
        // [B-DEBUG] 临时调试日志：确认 B 通道指令是否真的发出
        const bStrengthDebug = this.gameConfig.enableBChannel
            ? Math.min(strength * this.gameConfig.bChannelStrengthMultiplier, this.effectiveChannelBLimit)
            : null;
        console.log('[B-DEBUG] setClientStrength', {
            strength,
            enableBChannel: this.gameConfig.enableBChannel,
            multiplier: this.gameConfig.bChannelStrengthMultiplier,
            effALimit: this.effectiveChannelALimit,
            effBLimit: this.effectiveChannelBLimit,
            devB_limit: this.client?.strengthChannelB?.limit,
            bStrengthWillSend: bStrengthDebug,
        });

        const aStrength = Math.min(strength, this.effectiveChannelALimit);
        await this.client.setStrength(Channel.A, aStrength);
        if (this.gameConfig.enableBChannel) {
            let bStrength = Math.min(strength * this.gameConfig.bChannelStrengthMultiplier, this.effectiveChannelBLimit);
    ...
    private async runGameTask(ab: AbortController, harvest: () => void, round: number): Promise<void> {
        ...
        targetStrength = Math.min(targetStrength, this.effectiveChannelALimit);
    ...
    public async startGame(ignoreEvent = false): Promise<void> {
        ...
        if (!this.gameTask) {
            ...
            initStrength = Math.min(initStrength, this.effectiveChannelALimit); // 限制初始强度不超过限制
